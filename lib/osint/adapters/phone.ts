...
export const indonesiaPhoneAdapter: Adapter = {
  name: "Indonesia Phone OSINT v5-style",
  supports: ["phone"],
  async search(entity) {
    ...
    const queries = [`"${local}"`, `"${intl}"`, `"${plainIntl}"`];
    const searched = await Promise.allSettled(queries.map(searchWeb));
    const hits = searched.flatMap((x) => (x.status === "fulfilled" ? x.value : []));
    const seen = new Set<string>();
    const entities = [] as ReturnType<typeof extractFromText>;
    const findings = [{ title: `Provider prefix: ${provider}`, url: "", snippet: `${local} normalized as ${intl}`, source: "Indonesia Phone Prefix" }];
    for (const hit of hits) {
      ...
      findings.push({ title: hit.title || hit.url, url: hit.url, snippet: hit.snippet, source: `Phone Search/${hit.engine}` });
    }
